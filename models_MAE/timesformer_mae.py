...
Architecture:
- Encoder: Divided space-time TimeSformer (same as SimCLR)
- Decoder: Lightweight transformer
- Asymmetric masking: Heavy on before/after, light on threshold frames
"""
import torch
import torch.nn as nn
import torch.nn.functional as F
from masking import random_masking_asymmetric, patchify, unpatchify
...
class TimeSformerMAE(nn.Module):
    """
    MAE with TimeSformer encoder and lightweight decoder.
    """
    def __init__(self, config):
        ...
        # Encoder (standard transformer blocks - compatible with masked inputs)
    ...
    def forward_encoder(self, x, frame_mask_ratios):
        """
        Encode with asymmetric masking.

        Args:
            x: [B, T, C, H, W]
            frame_mask_ratios: Tuple of T mask ratios

        Returns:
            x_encoded: [B, N_keep, D] encoded visible patches
        ...
        x = tokens.reshape(B, T * self.S, -1)

        # Asymmetric masking
        x_masked, mask, ids_restore = random_masking_asymmetric(
            x, frame_mask_ratios, self.S
        )

        # Encoder transformer blocks (on visible patches only)
    ...
    def forward_decoder(self, x_encoded, ids_restore):
        ...
        Args:
            x_encoded: [B, N_keep, encoder_dim]
            ids_restore: [B, T*S]
        ...
        x = self.decoder_embed(x_encoded)  # [B, N_keep, decoder_dim]
        ...
        # Predict pixels
        pred = self.decoder_pred(x)  # [B, N, patch_pixels]
    ...
    def forward(self, imgs, frame_mask_ratios=None):
        """
        Forward pass: encode with masking, decode, compute loss.

        Args:
            imgs: [B, T, C, H, W]
            frame_mask_ratios: Tuple of T mask ratios (default: use config)
        ...
        Returns:
            ...
            mask: [B, T*S]
        """
        if frame_mask_ratios is None:
            frame_mask_ratios = self.config.frame_mask_ratios

        # Encode
        x_encoded, mask, ids_restore = self.forward_encoder(imgs, frame_mask_ratios)
